...
// Perform the database query to check if the model is available
$model_verify_query = "SELECT * FROM car WHERE model='$model'";
$model_verify_result = mysqli_query($con, $model_verify_query);

// Check if the query was successful
if ($model_verify_result) {
    // Check the number of rows in the result set
    if (mysqli_num_rows($model_verify_result) == 0) {
        echo "<script type='text/javascript'>alert('This Model Isn\'t Available');window.location='reserve.html?username=$username&pickupDate=$pickupDate&dropoffDate=$dropoffDate&city=$city&country=$country';</script>";
