...
$year_verify_query = "SELECT * FROM car WHERE `year`='$year'";
$year_verify_result = mysqli_query($con, $year_verify_query);

// Check if the query was successful
if ($year_verify_result) {
    // Check the number of rows in the result set
    if (mysqli_num_rows($year_verify_result) == 0) {
        echo "<script type='text/javascript'>alert('This Year Isn\'t Available');window.location='reserve.html?username=$username&pickupDate=$pickupDate&dropoffDate=$dropoffDate&city=$city&country=$country';</script>";
